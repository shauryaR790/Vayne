...
def _generate_title(
    ...
    *,
    ...
    if cluster_type == "asset":
        ...
        if exploit:
            return "Remote Code Execution"
        if any("secret" in str(f.get("title") or "").lower() for f in findings):
...
def _reason_exists(
    ...
    *,
    ...
    f = findings[0]
    return str(f.get("unique_reason") or f.get("why_it_matters") or "Retained evidence warrants analyst review.")
...
def _cluster_business_impact(findings: list[dict[str, Any]]) -> str:
    for f in findings:
        ...
        summary = str(detail.get("summary") or f.get("business_impact") or "").strip()
        if summary and "unknown" not in summary.lower() and "needs validation" not in summary.lower():
            return summary[:220]
    if _internet_facing(findings):
        return "Internet-facing exposure — confirm customer or partner data at risk."
